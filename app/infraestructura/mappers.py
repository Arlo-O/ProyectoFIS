...
# --- USUARIO (JOINED TABLE INHERITANCE - 3 NIVELES) ---
...
permiso_table = Table(
    ...
    Column('fecha_creacion', DateTime, default=lambda: datetime.now(timezone.utc)),
    Column('fecha_actualizacion', DateTime, default=lambda: datetime.now(timezone.utc), onupdate=lambda: datetime.now(timezone.utc)),
)
...
def start_mappers():
    ...
    try:
        ...
        # 5. ESTUDIANTE (hereda de Usuario)
        mapper_registry.map_imperatively(
            Estudiante,
            estudiante_table,
            inherits=Usuario,
        ...
        # 10. ADMINISTRADOR (hereda de Usuario)
        mapper_registry.map_imperatively(
            Administrador,
            administrador_table,
            inherits=Usuario,
